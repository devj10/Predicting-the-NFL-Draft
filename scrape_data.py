...
def parse_pfr_tables_by_season(tables) -> pd.DataFrame:
    results = []
    games = None
    seasons = None
    ...
    for table in tables:
        ...
        if table_id in HEADERS:
            try:
                ...
                tbody = table.find('tbody')
                if not tbody:
                    logging.warning(f"No tbody found in table {table_id}")
                    continue
                
                season_rows = [row for row in tbody.find_all('tr') 
                              if not row.get('class') or 'thead' not in row.get('class')]
                if not season_rows:
                    logging.warning(f"No season rows found in table {table_id}")
                    continue
                ...
                for i, row in enumerate(season_rows):
                    for td in row.find_all('td'):
                        ...
                        if stat_name and stat_name in autostat_list:
                            ...
                            if value:
                                try:
                                    stats[f'year.{i}.{stat_name}'] = float(value)
                                except ValueError:
                                    continue
    ...
    try:
        ...
        for i, url in enumerate(urls):
            try:
                ...
                stats = parse_pfr_tables_by_season(tables)
                if not stats.empty:
        ...
        college_stats_file = 'data/college_stat_by_season.feather'
