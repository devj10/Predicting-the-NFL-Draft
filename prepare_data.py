...
def load_raw_data(by_season: bool = False):
    ...
    if by_season:
        college_stats = pd.read_feather('data/college_stat_by_season.feather')
    else:
...
def save_processed_data(training, train_set, test_set, holdout_set, by_season: bool = False):
    """Save processed data and splits, with separate files for each draft year"""
    # Save the complete dataset
    if by_season:
        training.to_feather('data/processed/training_by_season.feather')
        pd.Series(train_set).to_csv('data/processed/train_set_by_season.csv', index=False)
        pd.Series(test_set).to_csv('data/processed/test_set_by_season.csv', index=False)
        pd.Series(holdout_set).to_csv('data/processed/holdout_set_by_season.csv', index=False)
    else:
...
def prepare_data(by_season: bool = True):
    """Main function to run the data preparation pipeline"""
    # Load data
    combine_table, draft_table, college_stats = load_raw_data(by_season=by_season)
    ...
    # Save data
    save_processed_data(training, train_set, test_set, holdout_set, by_season=by_season)
...
if __name__ == "__main__":
    training, train_set, test_set, holdout_set = prepare_data(by_season=False)
